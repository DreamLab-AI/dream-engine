...
export function checkDarwinBounds(
  ...
  if (perGen.size > DARWIN_BOUNDS.maxGenerations) {
    violations.push(`generations=${perGen.size} > ${DARWIN_BOUNDS.maxGenerations}`);
  }
  let maxCandidates = 0;
...
export function checkDarwinBoundsFromStdout(
  ...
  const rows = parseLeaderboardRows(stdout);
  const report = checkDarwinBounds(rows, promotedLineages);
  if (rows.length === 0) {
    return {
      ...report,
      ok: false,
      violations: ['leaderboard unparsable: 0 rows read', ...report.violations],
    };
  }
  return report;
}
